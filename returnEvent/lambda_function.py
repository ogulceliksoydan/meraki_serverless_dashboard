...
def lambda_handler(event, context):
    #return "User " + event["username"] + " created network " + event["netname"] + ", bound to template " + event["template"] + ", and added serials " + str(event["serials"])
    
    if event["netname"] == "": return "Please enter a network name"
    else:
    ...
    try:
        ...
        created = requests.request('POST', url=net_url, headers=headers, data=net_payload)
        ...
        requests.request('POST', url=bind_url, headers=headers, data=bind_payload)
    ...
    added = requests.request('POST', url=add_url, headers=headers, data=add_payload)
